...
for (i in 1:200){prop10.na.geopt2.list[[i]] <- fun.mval(list.sim$sim_list_GeoPT2.RData[[i]],0.2)}
...
geombias.mse.na.fun <- function(true,list.df){  
  ...
  ilr.mean.df <- lapply(lapply(lapply(imputed.df,ilr),colMeans),ilrInv) %>% bind_rows() %>% mutate(type="ilr mean") 
  names(ilr.mean.df) <- names(blr.df)
  
  alr.mean.df <- lapply(lapply(imputed.df,alr),colMeans) %>% bind_rows() %>% alrInv() %>% as_tibble() %>% mutate(type="alr mean") 
  names(alr.mean.df) <- names(blr.df)
  
  clr.mean.df <- lapply(lapply(imputed.df,clr),colMeans) %>% bind_rows() %>% clrInv() %>% as_tibble() %>% mutate(type="clr mean") 
  names(clr.mean.df) <- names(blr.df)
  
  
  geometric.mean <- lapply(imputed.df,geometric.mean.na) %>% bind_rows() %>% mutate(type="geometric mean")
  
  
  combined.df <- bind_rows(opti.blr.df,blr.df,col.mean.df,closed.blr,ilr.mean.df,geometric.mean,clr.mean.df,alr.mean.df)
...
output.geopt2 <- geombias.mse.na.fun(true = mean.compo$GeoPT2.csv,      list.df = prop10.na.geopt2.list)
output.geopt20 <- geombias.mse.na.fun(true = mean.compo$`GeoPT20 .csv`, list.df = prop10.na.geopt20.list)
output.geopt22 <- geombias.mse.na.fun(true = mean.compo$`GeoPT22 .csv`, list.df = prop10.na.geopt22.list)
output.geopt25 <- geombias.mse.na.fun(true = mean.compo$`GeoPT25 .csv` ,list.df = prop10.na.geopt25.list)
output.geopt32 <- geombias.mse.na.fun(true = mean.compo$`GeoPT32 .csv` ,list.df = prop10.na.geopt32.list)
output.geopt35 <- geombias.mse.na.fun(true = mean.compo$`GeoPT35 .csv` ,list.df = prop10.na.geopt35.list)
output.geopt36 <- geombias.mse.na.fun(true = mean.compo$`GeoPT36 .csv` ,list.df = prop10.na.geopt36.list)
output.geopt41 <- geombias.mse.na.fun(true = mean.compo$`GeoPT41 .csv` ,list.df = prop10.na.geopt41.list)
output.geopt46 <- geombias.mse.na.fun(true = mean.compo$`GeoPT46 .csv` ,list.df = prop10.na.geopt46.list)
...
saveRDS(combined_tib,file = "missingvalues/prop10/combined_tib.RData")
...
saveRDS(total.bias,file = "missingvalues/prop10/totalbias.RData")
