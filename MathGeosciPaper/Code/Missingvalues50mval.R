...
prop50.na.geopt38.list <- list()
prop50.na.geopt41.list <- list()
prop50.na.geopt46.list <- list()

geopt2.prop50.corrected <- list()
...
for (i in 1:200){prop50.na.geopt38.list[[i]] <- fun.mval(list.sim$sim_list_GeoPT38.RData[[i]],0.5)}
for (i in 1:200){prop50.na.geopt41.list[[i]] <- fun.mval(list.sim$sim_list_GeoPT41.RData[[i]],0.5)}
for (i in 1:200){prop50.na.geopt46.list[[i]] <- fun.mval(list.sim$sim_list_GeoPT46.RData[[i]],0.5)}

setwd("~/Documents/GeoPTManuscript/MathGeosciPaper/Code/")
saveRDS(object = prop50.na.geopt2.list,file = "missingvalues/prop50/prop50_geopt2.RData")
saveRDS(object = prop50.na.geopt20.list,file = "missingvalues/prop50/prop50_geopt20.RData")
saveRDS(object = prop50.na.geopt22.list,file = "missingvalues/prop50/prop50_geopt22.RData")
saveRDS(object = prop50.na.geopt25.list,file = "missingvalues/prop50/prop50_geopt25.RData")
saveRDS(object = prop50.na.geopt32.list,file = "missingvalues/prop50/prop50_geopt32.RData")
saveRDS(object = prop50.na.geopt35.list,file = "missingvalues/prop50/prop50_geopt35.RData")
saveRDS(object = prop50.na.geopt36.list,file = "missingvalues/prop50/prop50_geopt36.RData")
saveRDS(object = prop50.na.geopt38.list,file = "missingvalues/prop50/prop50_geopt38.RData")
saveRDS(object = prop50.na.geopt41.list,file = "missingvalues/prop50/prop50_geopt41.RData")
saveRDS(object = prop50.na.geopt46.list,file = "missingvalues/prop50/prop50_geopt46.RData")
...
output.geopt2 <-  geombias.mse.fun(true = mean.compo$GeoPT2.csv,list.df = prop50.na.geopt2.list)
output.geopt20 <- geombias.mse.fun(true = mean.compo$`GeoPT20 .csv`,list.df = prop50.na.geopt20.list)
output.geopt22 <- geombias.mse.fun(true = mean.compo$`GeoPT22 .csv`,list.df = prop50.na.geopt22.list)
output.geopt25 <- geombias.mse.fun(true = mean.compo$`GeoPT25 .csv` ,list.df = prop50.na.geopt25.list)
output.geopt32 <- geombias.mse.fun(true = mean.compo$`GeoPT32 .csv` ,list.df = prop50.na.geopt32.list)
output.geopt35 <- geombias.mse.fun(true = mean.compo$`GeoPT35 .csv` ,list.df = prop50.na.geopt35.list)
output.geopt36 <- geombias.mse.fun(true = mean.compo$`GeoPT36 .csv` ,list.df = prop50.na.geopt36.list)
output.geopt41 <- geombias.mse.fun(true = mean.compo$`GeoPT41 .csv` ,list.df = prop50.na.geopt41.list)
output.geopt46 <- geombias.mse.fun(true = mean.compo$`GeoPT46 .csv` ,list.df = prop50.na.geopt46.list)


geommeandf <- function(dataset){
  dataset <- as_tibble(dataset)
  df <- dataset[-ncol(dataset)] %>% rowMeans() %>% as_tibble()
  df[ncol(df)+1] <- dataset[ncol(dataset)]
  return(df)
}

list.output <- list(output.geopt2 [[1]],
                    output.geopt20[[1]],
                    output.geopt22[[1]],
                    output.geopt25[[1]],
                    output.geopt32[[1]],
...
combined_tib %>% ggplot(aes(x=dataset,y=value,color=type))+geom_boxplot()+
  ...
  labs(y="Empirical Mean Bias")+scale_y_log10()




###############
# BOX PLOT Geometric Mean Bias 
###########
...
saveRDS(combined_tib,file = "missingvalues/prop50/combined_tib.RData")
######################""
...
saveRDS(total.bias,file = "missingvalues/prop50/totalbias.RData")