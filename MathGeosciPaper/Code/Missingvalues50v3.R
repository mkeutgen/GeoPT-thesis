...
saveRDS(object = prop50.na.geopt1.list  , file = "missingvalues/prop50/prop50_geopt1.RData")
...
saveRDS(object = prop50.na.geopt21.list , file = "missingvalues/prop50/prop50_geopt21.RData")
...
saveRDS(object = prop50.na.geopt3.list  , file = "missingvalues/prop50/prop50_geopt3.RData")
saveRDS(object = prop50.na.geopt32.list , file = "missingvalues/prop50/prop50_geopt32.RData")
saveRDS(object = prop50.na.geopt34.list , file = "missingvalues/prop50/prop50_geopt34.RData")
...
geombias.mse.fun <- function(true,list.df){  
  ...
  mse.df <- (sweep(logit(combined.df[1:21]), 2, logit(true[1:21]), "-")^2)
...
geopt1.o <- geombias.mse.fun(true = mean.compo$GeoPT1.csv,  list.df  = prop50.na.geopt1.list)
geopt2.o <- geombias.mse.fun(true=mean.compo$GeoPT2.csv,    list.df  = prop50.na.geopt2.list)
geopt20.o <- geombias.mse.fun(true=mean.compo$`GeoPT20 .csv`,list.df = prop50.na.geopt20.list)
geopt21.o <- geombias.mse.fun(true=mean.compo$`GeoPT21 .csv`,list.df = prop50.na.geopt21.list)
geopt22.o <- geombias.mse.fun(true=mean.compo$`GeoPT22 .csv`,list.df = prop50.na.geopt22.list)
geopt25.o <- geombias.mse.fun(true=mean.compo$`GeoPT25 .csv`,list.df = prop50.na.geopt25.list)
geopt3.o <- geombias.mse.fun(true=mean.compo$GeoPT3.csv,     list.df = prop50.na.geopt3.list)
geopt32.o <- geombias.mse.fun(true=mean.compo$`GeoPT32 .csv`,list.df = prop50.na.geopt32.list)
geopt34.o <- geombias.mse.fun(true=mean.compo$`GeoPT34 .csv`,list.df = prop50.na.geopt34.list)
geopt35.o <- geombias.mse.fun(true=mean.compo$`GeoPT35 .csv`,list.df = prop50.na.geopt35.list)
geopt36.o <- geombias.mse.fun(true=mean.compo$`GeoPT36 .csv`,list.df = prop50.na.geopt36.list)
...
combined_tib %>% ggplot(aes(x=dataset,y=value,color=type))+geom_boxplot()+
  theme_bw()+theme(legend.position = "bottom",axis.text.x = element_text(angle = 45, hjust = 1))+labs(y="Boxplot of Mean Bias")+scale_y_log10()
